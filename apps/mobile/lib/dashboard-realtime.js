// apps/mobile/lib/dashboard-realtime.js
// Phase 7 Plan 07-07 — Dashboard-summary Realtime subscription helper
// (DASH-03 / D-02 / D-03).
//
...
// Detects re-subscription transitions and invokes onReconnect so the Home
// screen can refetch the summary (D-03 reconcile-on-reconnect).
//
// Channel naming: `dashboard:<societyId>`. We deliberately use a NEW channel
// name (not piggy-backing `complaints-${societyId}` etc.) because:
//   - The dashboard watches 8 tables; the per-feature channels watch one each
//     and are scoped to their screen's lifecycle (`useFocusEffect` cleanup).
//     A new dashboard channel decouples the Home subscription from the feature
//     screens' subscribe/teardown cycle.
//   - Channel names are namespaces — collisions don't cause cross-talk, just
//     extra wire traffic and confusing debug logs.
//
// Security (07-07-PLAN <threat_model> + <security_contract>):
...
//     reconnect happened (network drop, app suspend/resume on iOS, etc.).
//   - When we observe the second-and-later SUBSCRIBED, fire onReconnect().
//   - We do NOT debounce reconnect here — the Home screen owns refetch
//     cadence; Supabase Realtime itself debounces reconnect attempts.
//
// JavaScript only — no TypeScript per CLAUDE.md.

// The 8 source tables that feed the dashboard tiles (07-07-PLAN must_haves
// truth #3). Order is informational only — events are dispatched independently.
 ...
 *   onReconnect — the caller already loaded its initial data via fetchSummary.
 * @returns {() => void}
