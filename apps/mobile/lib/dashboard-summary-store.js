// apps/mobile/lib/dashboard-summary-store.js
// Phase 7 Plan 07-07 — Zustand store for the live dashboard summary (DASH-03).
//
...
// The Home screen calls `fetchSummary(supabase)` on every focus
// (D-03 reconciliation) and routes per-table INSERT/UPDATE events through
// `patchEvent` so counts + previews tick without waiting for a full refetch.
//
// Threat model (07-07-PLAN <threat_model>):
...
//   - T-07-04 (Cross-society leakage): backend RLS is authoritative; this
//     reducer does NOT trust the event payload's `society_id` — the dashboard
...
export const useDashboardSummary = create((set, get) => ({
  ...
  /**
   * Replace the summary atomically (used by reconnect-reconcile and tests).
