// apps/web/__tests__/web-dashboard-summary-store.test.js
// Phase 7 — Plan 07-08 Task 1 (TDD RED→GREEN).
//
// Web port of apps/mobile/__tests__/dashboard-summary-store.test.js (Plan 07-07).
// 1:1 behaviour parity — the web store mirrors the mobile store contract so
// future code can be lifted between platforms without surface drift.
//
// Behaviours under test (per 07-08-PLAN.md must_haves):
...
//   5. patchEvent INSERT complaint, member view + reporter_id matches:
//      myComplaints.count++ (NOT complaints.count).
//   6. patchEvent INSERT complaint, member view + reporter_id mismatch: no change.
...
//      community.previews and decrements community.count.
//   10. reconcile: fetchSummary after a patch sequence replaces summary atomically.
//
// Bonus tests (T-07-27 reducer safety + no-summary safety) also ported from
// the mobile suite so the web store has the same defensive surface.
//
...
// success/failure cases without a live Supabase. Capture the spy so individual
// tests can mock the next resolution.
...
describe("web dashboard-summary-store (Phase 7 Plan 07-08)", () => {
  beforeEach(() => {
  ...
  it("patchEvent UPDATE post hidden_at: drops matching preview AND decrements community.count", () => {
    ...
    expect(s.community.count).toBe(1);
  });
