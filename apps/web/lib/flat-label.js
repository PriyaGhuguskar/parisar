// apps/web/lib/flat-label.js
// Web mirror of apps/mobile/lib/flat-label.js. Same Supabase schema, same
// PostgREST FK-embed query — no client-specific differences.
//
// Resolve "{wing}-{number}" flat label for a user when JWT app_metadata
// does not carry flat_label (it doesn't — see 04.1-00-JWT-SHAPE.md).
// Falls back to "—" when no membership or query failure.
//
// JavaScript only — no TypeScript per CLAUDE.md.

/**
 * @param {object} supabase - authenticated supabase-js client (browser or server)
 * @param {string} userId
